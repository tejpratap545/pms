	...
	<div class="page">
		...
		<div v-else class="center">
			<vs-navbar v-model="active" center-collapsed>
				<vs-navbar-item id="department-details" :active="active == 'department-details'">
					Department details
				</vs-navbar-item>
				<vs-navbar-item id="subordinate-details" :active="active == 'subordinate-details'">
					Subordinate details
				</vs-navbar-item>
				<vs-navbar-item id="subordinate-appraials" :active="active == 'subordinate-appraials'">
			...
			<div class="square">
				<div v-if="active == 'department-details'" class="child">
					<div class="status-user-profile">
						<h3 style="margin-top: 10px; text-align: center">Department details</h3>
						<div class="details">
							<vs-table class="table" striped>
								<template #tbody>
									<vs-tr>
										...
										<vs-td>
											{{ $store.state.user.department ? $store.state.user.department.name : '' }}
										</vs-td>
									</vs-tr>
									<vs-tr>
					...
					<div>
						<vs-row v-if="apprisalList.length != 0">
									...
									<template #expand>
										<div>
											<div class="con-content">
												<vs-button color="success" flat icon @click="editDepartmentGoal = true">
													<i class="bx bx-edit-alt"></i>
												</vs-button>
												<vs-button border danger @click="deleteItem('departmental-goal', tr.id)">
													Delete
												</vs-button>
											</div>
										</div>
						...
						<vs-table v-model="selected" class="my-5">
							<template #header>
								<div class="table-header" style="justify-content: space-between">
									...
									<vs-button @click="newDepartmentCoreValue = true"> Add </vs-button>
								</div>
							</template>
							<template #thead>
								<vs-tr>
									...
									<template #expand>
										<div>
											<vs-table class="my-5">
												...
												<template #tbody>
													<vs-tr v-for="(appraisal, j) in tr.appraisal_set" :key="j">
														<vs-td style="padding: 10px">{{ appraisal.name }}</vs-td>
														<vs-td>{{ appraisal.goal_count }}</vs-td>
														<vs-td>{{ appraisal.corevalue_count }}</vs-td>
														<vs-td>{{ appraisal.skill_count }}</vs-td>
														<vs-td>
															<i> {{ getStatus(appraisal.status, appraisal.stage) }}</i></vs-td
														>
													</vs-tr>
												</template>
											</vs-table>
					...
					<div class="square">
						<div v-if="activeStage == 'stage1'" class="child">
							<vs-table v-model="selected">
								...
								<template #tbody>
									<vs-tr
										v-for="(tr, i) in $vs.getSearch(appraisalManagerList.stage1, search)"
										:key="i"
										:data="tr"
										:is-selected="selected == tr"
									>
											...
											<div class="con-content">
												<div>
													<vs-button
														...
														flat
														icon
														@click="(appraisalId = tr.id), (approveAppraisal = true)"
													>
														<i class="bx bx-edit-alt"></i>
													</vs-button>
												</div>
						...
						<div v-if="activeStage == 'stage2'" class="child">
							<vs-table v-model="selected">
								...
								<template #tbody>
									<vs-tr
										v-for="(tr, i) in $vs.getSearch(appraisalManagerList.stage2, search)"
										:key="i"
										:data="tr"
										:is-selected="selected == tr"
									>
										...
										<template #expand>
											<div class="con-content">
												<div>
													<vs-button
														v-if="(tr.status === 4 || tr.status === 5) && tr.stage === 1"
														color="success"
														flat
														icon
													...
													<vs-button
														...
														flat
														icon
														@click="(appraisalId = tr.id), (midReviewApprove = true)"
													>
														approve
													</vs-button>
												</div>
						...
						<div v-if="activeStage == 'stage3'" class="child">
							<vs-table v-model="selected">
								...
								<template #tbody>
									<vs-tr
										v-for="(tr, i) in $vs.getSearch(appraisalManagerList.stage3, search)"
										:key="i"
										:data="tr"
										:is-selected="selected == tr"
									>
											...
											<div class="con-content">
												<div>
													<div class="con-content">
														<div>
															<vs-button
																v-if="(tr.status === 8 || tr.status === 9) && tr.stage === 2"
																color="success"
																flat
																icon
																@click="(appraisalId = tr.id), (endReview = true)"
															>
																end review
															</vs-button>
															<vs-button
																...
																flat
																icon
																@click="(appraisalId = tr.id), (endReviewApprove = true)"
															>
																approve
															</vs-button>
														</div>
			...
			<NewDepartmentGoalDialog
				...
				:selected-appraisal="apprisalList.filter((x) => x.id == selectedAppraisalId)[0]"
...
export default {
	...
	async fetch() {
		try {
			...
			if (this.apprisalList.length !== 0) this.selectedAppraisalId = this.apprisalList[0].id;
			...
			this.userManagerList = await this.$axios.$get(`api/appraisal/short/manager`, {
				headers: {
					Authorization: `Bearer ${this.$store.state.accessToken}`
				}
			});
			...
			this.departmentCoreValue = await this.$axios.$get(`api/departmental-core-value/`, {
				headers: {
					Authorization: `Bearer ${this.$store.state.accessToken}`
				}
			});
