		...
		<div v-else class="center grid">
			<div class="appraisal-lists">
				...
				<div
					...
					:key="a.id"
					:class="`appraisal-list-item ${selectedAppraisal.id == a.id ? `active` : ``}`"
					...
					<div class="appraisal-item">
						<div>
							...
							<vs-row class="my-5 pa-2">
								...
								<vs-col w="8" style="display: flex; justify-content: flex-end">
									<vs-button v-if="stage == 0 && status == 0" @click="goalSubmit = true">
										Submit Goal
									</vs-button>
									<vs-button v-if="stage == 1 && status == 2" @click="midyearReview = true">
										Midyear Review
									</vs-button>
									<vs-button v-if="stage == 1 && status == 3" @click="midyearReview = true">
										Edit Midyear Review
									</vs-button>
									<vs-button v-if="stage == 1 && status == 3" @click="submitMidyearReview = true">
										Submit Midyear Review
									</vs-button>
									<vs-button v-if="stage == 2 && status == 6" @click="endyearReview = true">
										EndYear Review
									</vs-button>
									<vs-button v-if="stage == 2 && status == 7" @click="endyearReview = true">
										Edit End Year Review
									</vs-button>
									<vs-button v-if="stage == 2 && status == 7" @click="submitEndyearReview = true">
										Submit End Year Review
									</vs-button>
									<vs-button icon @click="location.replace(`/print?id=${selectedAppraisal.id}`)">
						...
						<Appraisal :edit="true" :appraisal="currentAppraisal" @refresh="$fetch()" />
					</div>
				</div>
				<div v-else>
					<h2>No Appraisals</h2>
...
export default {
	...
	async fetch() {
		try {
			...
			if (this.apprisalList.length !== 0) this.selectedAppraisal = this.apprisalList[0];
		} catch (err) {
