	...
	<div class="page">
		...
		<div v-else class="center grid">
			<vs-table v-model="selected">
				...
				<template #thead>
					<vs-tr>
						<vs-th sort @click="appraisalList = $vs.sortData($event, appraisalList, 'name')">
							Name
						</vs-th>
						<vs-th sort @click="appraisalList = $vs.sortData($event, appraisalList, 'status')">
							Stage
						</vs-th>
						<vs-th
							v-if="$store.state.user.user.is_superuser"
							sort
							@click="appraisalList = $vs.sortData($event, appraisalList, 'company')"
						>
							company
						</vs-th>
						<vs-th sort @click="appraisalList = $vs.sortData($event, appraisalList, 'id')">
							Id
						</vs-th>
					</vs-tr>
				</template>
	...
	data: () => ({
		...
		steps: [
			...
			},
			{
				...
				content: 'Use this button to add appraisals for individuals, departments or entire company',
				params: {
					placement: 'top'
				}
			},
