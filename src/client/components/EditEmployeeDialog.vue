<template>
	<vs-dialog v-model="active" :loading="loading" not-close prevent-close>
		...
		<div v-else class="con-form">
			...
			<vs-input v-model="employeeData.user.password" type="password" placeholder="User password">
				<template #icon>
		...
		<template #footer>
			<div v-if="validEmail && validUsername" class="footer-dialog">
				<vs-button :loading="loading" block @click="updateEmployee(employeeData.id)">
					Update
				</vs-button>
			</div>
			<div v-else>
...
export default {
	...
	async fetch() {
		...
		try {
			...
			this.employeeData = await this.$axios.$get(`api/user/${this.selectedEmployee.id}/`, {
				headers: {
					Authorization: `Bearer ${this.$store.state.accessToken}`
				}
			});
			...
			this.employeeData.first_reporting_manager = this.employeeData.first_reporting_manager.id;
			this.employeeData.second_reporting_manager = this.employeeData.second_reporting_manager.id;
