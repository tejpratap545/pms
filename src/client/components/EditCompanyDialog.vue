<template>
	<vs-dialog v-model="active" not-close prevent-close>
		...
		<div class="con-form">
			<vs-input v-model="companyData.admin_username" placeholder="Admin username">
				<template #icon>
					<i class="bx bx-user"></i>
				</template>
			</vs-input>

			<vs-input v-model="companyData.admin_password" type="password" placeholder="Admin password">
				<template #icon>
			...
			<vs-input v-model="companyData.domain" type="url" placeholder="Company domain">
				<template #icon> <i class="bx bx-globe"></i> </template>
			</vs-input>

			<h3 style="text-align: center">Nomenclature</h3>
			<div v-if="companyData.setting != null">
				<vs-input
					v-model="companyData.setting.goal_nomenclature"
					type="url"
				...
				<vs-input
					v-model="companyData.setting.corevalue_nomenclature"
					type="url"
				...
				<vs-input
					v-model="companyData.setting.skills_nomenclature"
					type="url"
		...
		<template #footer>
			<div class="footer-dialog">
				<vs-button :loading="loading" block @click="updateCompany(companyData.id)">
					Update
				</vs-button>
			</div>
		</template>
...
export default {
	...
	mounted() {
		...
		if (this.companyData.setting == null) {
			this.companyData.setting = {
				goal_nomenclature: '',
