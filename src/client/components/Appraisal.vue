<template>
	<div>
		<vs-table v-model="selected" class="my-5">
			...
			<template #tbody>
				<vs-tr
					v-for="(tr, i) in appraisal.goal_set"
					:key="i"
					:data="tr"
					...
					<template #expand>
						<div>
							<div class="con-content">
								<vs-button v-if="CanEdit" color="success" flat icon @click="editGoal = true">
									<i class="bx bx-edit-alt"></i>
								</vs-button>
								<vs-button v-if="CanEdit" color="success" flat icon @click="chatEmployee = true">
									<i class="bx bx-chat"></i>
								</vs-button>
								<vs-button v-if="goalApprove" color="success" flat icon @click="chatManager = true">
									<i class="bx bx-chat"></i>
								</vs-button>
								<vs-button v-if="CanEdit" border danger @click="deleteItem('goal', tr.id)">
									Delete
								</vs-button>
								<vs-button
									v-if="goalApprove"
		...
		<vs-table v-model="selected" class="my-5">
			<template #header>
				<div class="table-header" style="justify-content: space-between">
					<h3>Core values</h3>
					<vs-button v-if="CanEdit" @click="newCoreValue = true"> Add </vs-button>
				</div>
			</template>
			<template #thead>
				<vs-tr>
			...
			<template #tbody>
				<vs-tr
					v-for="(tr, i) in appraisal.corevalue_set"
					:key="i"
					:data="tr"
					:is-selected="selected == tr"
				>
					...
					<template #expand>
						<div>
							<div v-if="CanEdit" class="con-content">
								<vs-button color="success" flat icon @click="editCoreValue = true">
									<i class="bx bx-edit-alt"></i>
								</vs-button>
								<vs-button border danger @click="deleteItem('core-value', tr.id)">
									Delete
								</vs-button>
							</div>
						</div>
		...
		<vs-table v-model="selected" class="my-5">
			...
			<template #tbody>
				<vs-tr
					v-for="(tr, i) in appraisal.skill_set"
					:key="i"
					:data="tr"
					:is-selected="selected == tr"
				>
					...
					<template #expand>
						<div>
							<div v-if="CanEdit" class="con-content">
								...
								<vs-button border danger @click="deleteItem('skill', tr.id)"> Delete </vs-button>
							</div>
						</div>
					</template>
				</vs-tr>
		...
		<vs-table class="my-5">
			...
			<template #tbody>
				<vs-tr v-for="(tr, i) in appraisal.overall_appraisal.departmentalgoal_set" :key="i">
					<vs-td>
						{{ tr.summary }}
					</vs-td>
					<vs-td>
		...
		<vs-table class="my-5">
			...
			<template #tbody>
				<vs-tr v-for="(tr, i) in appraisal.overall_appraisal.departmentalcorevalue_set" :key="i">
					<vs-td>
						{{ tr.summary }}
					</vs-td>
					<vs-td>
		...
		<GoalCommentEmployee
			...
			:dialog="chatEmployee"
			:selected-goal="selected"
			@close="(chatEmployee = false), $emit('refresh')"
		/>
		...
		<GoalCommentManager
			...
			:dialog="chatManager"
			:selected-goal="selected"
			@close="(chatManager = false), $emit('refresh')"
		/>
...
export default {
	...
	computed: {
		...
		CanEdit() {
			if (
				this.edit === true &&
				this.appraisal.employee.id === this.user.id &&
				this.appraisal.status === 0 &&
				this.appraisal.overall_appraisal.stage === 0
			) {
				return true;
			} else {
				return false;
