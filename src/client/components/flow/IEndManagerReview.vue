<template>
	<vs-dialog v-model="active" :loading="loading" not-close prevent-close>
		...
		<div v-else class="con-form">
									...
									<vs-row style="padding: 20px 0">
										...
										<vs-col w="8" class="description-card">
											<Editor
												...
												@changeData="(value) => (tr.stage2_manager_comment = value)"
											/>
										</vs-col>
									</vs-row>
			...
			<vs-col w="8">
				<span v-if="appraisal.core_value_employee_comment != null">
					<!-- eslint-disable-next-line vue/no-v-html -->
					<span v-html="appraisal.core_value_employee_comment"></span>
				</span>
				<span v-else>No Comment</span>
			</vs-col>

			<vs-col w="4">
				<b>Endyear manager core value comment</b>
			</vs-col>
			<vs-col w="8">
				<Editor
					:data="core_value_manager_comment"
					@changeData="(value) => (core_value_manager_comment = value)"
				/>
			</vs-col>

			<vs-row style="padding: 20px 0">
				<vs-col w="4">
					<b>Endyear manager core value rating</b>
				</vs-col>
				<vs-col w="8" style="display: flex; justify-content: flex-end">
					<star-rating
		...
		submit() {
			...
			this.$axios
				.$post(
					...
					{
						core_value_manager_comment: this.core_value_manager_comment,
						core_value_manager_rating: this.core_value_manager_rating
					},
					{
						headers: {
							Authorization: `Bearer ${this.$store.state.accessToken}`
