	...
	<vs-dialog v-model="active" :loading="loading" scroll overflow-hidden not-close prevent-close>
		...
		<div class="con-form">
			...
			<vs-row style="padding: 20px 0">
				...
				<vs-col w="8" style="display: flex; justify-content: flex-end">
					<star-rating
						v-model="appraisal.core_value_manager_rating"
						:star-size="20"
						:rating="appraisal.core_value_manager_rating"
					></star-rating>
				</vs-col>
			</vs-row>
		</div>
		...
		async submit() {
			...
			await this.$$axios.$patch(
				`api/appraisal/${this.appraisal.id}/`,
				{
					...
					core_value_employee_rating: this.appraisal.core_value_employee_rating
				},
				{
					headers: {
