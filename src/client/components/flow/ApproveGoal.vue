  ...
  <vs-dialog v-model="active" not-close prevent-close>
    ...
    <template #footer>
      <div class="footer-dialog">
        <vs-button :loading="loading" @click="actionGoal('approve')">
          Reject Goal
        </vs-button>
        <vs-button :loading="loading" @click="actionGoal('reject')">
          Approve Goal
        </vs-button>
      </div>
    </template>
...
export default {
  ...
  methods: {
    ...
    actionGoal(action) {
      this.$axios
        .$post(`api/goal/${this.goal.id}/up-stage/${action}`, {
          headers: {
