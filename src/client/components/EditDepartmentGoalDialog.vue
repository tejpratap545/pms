	...
	<vs-dialog v-model="active" :loading="loading" not-close prevent-close>
		...
		<template #footer>
			<div class="footer-dialog">
				<vs-button :loading="loading" block @click="updateGoal"> Update </vs-button>
			</div>
		</template>
	</vs-dialog>
</template>
	...
	methods: {
		...
		updateGoal() {
			if (!this.loading) {
				...
				this.$axios
					.$patch(`api/departmental-goal/${this.goalData.id}/`, this.goalData, {
						headers: {
							Authorization: `Bearer ${this.$store.state.accessToken}`
						}
					})
