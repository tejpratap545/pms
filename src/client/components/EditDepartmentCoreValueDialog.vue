	...
	<vs-dialog v-model="active" :loading="loading" not-close prevent-close>
		...
		<template #footer>
			<div class="footer-dialog">
				<vs-button :loading="loading" block @click="updateCoreValue"> Update </vs-button>
			</div>
		</template>
	</vs-dialog>
</template>
...
export default {
	...
	async fetch() {
		this.loading = true;
		try {
			this.coreValueCategoryList = await this.$axios.$get(`api/category/core_value/`, {
				headers: {
	...
	methods: {
		...
		updateCoreValue() {
			if (!this.loading) {
				...
				this.$axios
					.$patch(`api/departmental-core-value/${this.coreValueData.id}/`, this.coreValueData, {
						headers: {
							Authorization: `Bearer ${this.$store.state.accessToken}`
						}
					})
